...
with sensor.lr_session() as session:	# this will wait until the Live Response session is established
	try:
		session.put_file(open("cblr.bat", "r"), "c:\\cblr.bat")
	...
	output = session.create_process("cmd.exe c:\\cblr.bat")	# output has the stdout from running c:\test.bat
	print "output:"
	print output
	cblr_results = session.get_file("c:\\cblr.txt")
	print "cblr results"
	print cblr_results
